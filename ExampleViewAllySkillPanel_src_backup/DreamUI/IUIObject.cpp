...
void UI_Tick (Timer *tm)
{
	...
	{
		if(*(uint32_t *)(&iter) != NULL)//TODO crash
		{
			if (*iter)
			{
				(*iter)->tick();
...
{
	Color c(this);
	c.a = (uint8_t)(((uint32_t)c.a) * ( ((uint32_t)other.a) / ((uint32_t)0xFF) ));
	c.r = (uint8_t)(((uint32_t)c.r) * ( ((uint32_t)other.r) / ((uint32_t)0xFF) ));
	c.g = (uint8_t)(((uint32_t)c.g) * ( ((uint32_t)other.g) / ((uint32_t)0xFF) ));
	c.b = (uint8_t)(((uint32_t)c.b) * ( ((uint32_t)other.b) / ((uint32_t)0xFF) ));
	return c;
}
...
{
	//AARRGGBB
	return (uint32_t(a)<<0x18) | (uint32_t(r)<<0x10) | (uint32_t(g)<<0x8) | (uint32_t(b));
}
