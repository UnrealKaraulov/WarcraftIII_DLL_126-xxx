...
//  Thai                    = 0x0000041e
	LANGID langid = DreamStorm::FileGetLocale();
	switch (langid){
	case 0x00000409:
		return 0;
	case 0x00000809:
...
uint32_t GetUnitFramePoint(war3::CUnit* unit, FramePoint* point) {
	...
	return dreamaero::generic_fast_call<uint32_t>(Offset(UNIT_FRAMEPOSITION_GET), unit, point, &unk);
}
...
char *GetIconFromFuncProfile(uint32_t typeId, uint32_t index){
	...
	rv = dreamaero::generic_this_call<char *>(GetIcon, *(DWORD*)GetIconObj, typeIdStr, "Art", 0, index);
	return rv;
}
...
char *GetBuffIconFromFuncProfile(uint32_t typeId, uint32_t index){
	...
	char *rv;
	const char *typeIdStr = IntegerIdToChar(typeId);
	rv = dreamaero::generic_this_call<char *>(GetIcon, *(DWORD*)GetIconObj, typeIdStr, "Buffart", 0, index);
	return rv;
}
...
void GetIconPos(uint32_t typeId, uint32_t &x, uint32_t &y)//FIXME
{
	if (!typeId) return;
	const char *typeIdStr = IntegerIdToChar(typeId);
	char *posXstr = dreamaero::generic_this_call<char *>(GetIcon, *(DWORD*)GetIconObj, typeIdStr, "Buttonpos", 0, 0);
	char *posYstr = dreamaero::generic_this_call<char *>(GetIcon, *(DWORD*)GetIconObj, typeIdStr, "Buttonpos", 0, 1);
	x = posXstr ? atoi(posXstr) : 0;
	y = posYstr ? atoi(posYstr) : 0;
}

uint32_t GetAgentAgileDataHashKey(int agent_id) {
	return dreamaero::generic_fast_call<uint32_t>(Offset(AGILEDATA_HASHKEY_GET), &agent_id);
}
