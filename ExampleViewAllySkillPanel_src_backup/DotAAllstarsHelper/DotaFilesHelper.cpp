...
vector<unsigned char> FullPatchData;

void ProcessNodeAnims( unsigned char * ModelBytes, size_t _offset, vector<int *> & TimesForReplace )
{
	Mdx_Track tmpTrack;
...
BOOL __stdcall IsLocalFilesEnabled( int )
{
	if ( GameDll )
		return *( BOOL* )GameDll + 0xAAE2AC;
	return FALSE;
}

typedef void( __fastcall * EnableLocalFiles_p )( BOOL enable );
...
BOOL __stdcall CheckWriteAccess( int  )
{
	FILE * f = NULL;
