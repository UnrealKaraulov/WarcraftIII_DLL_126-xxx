...
int GetProcCount( )
{
	...
	for ( int i = 0; i < 32; i++ )
	{
		...
		long lError = RegOpenKeyExA( HKEY_LOCAL_MACHINE,
			( "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\" + to_string( i ) ).c_str( ),
			0,
			KEY_READ,
			&hKey );
...
std::string ProcSpeedRead( int id )
{
	...
	long lError = RegOpenKeyExA( HKEY_LOCAL_MACHINE,
		( "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\" + to_string( id ) ).c_str( ),
		0,
		KEY_READ,
		&hKey );
...
const char * __stdcall GetSystemInfo2( SystemInfo info, unsigned int DIV/*or arg2*/ )
{
	...
	case SystemInfo::PHYS_BYTES:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullTotalPhys / DIV ) );
		break;
	case SystemInfo::VIRTUAL_BYTES:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullTotalVirtual / DIV ) );
		break;
	case SystemInfo::PAGE_BYTES:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullTotalPageFile / DIV ) );
		break;
	case SystemInfo::PHYS_BYTES_CUR:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullAvailPhys / DIV ) );
		break;
	case SystemInfo::VIRTUAL_BYTES_CUR:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullAvailVirtual / DIV ) );
		break;
	case SystemInfo::PAGE_BYTES_CUR:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.ullAvailPageFile / DIV ) );
		break;
	case SystemInfo::PHYS_LOAD_PERCENT:
		sprintf_s( GlobalBuf, "%u", ( DWORD )( statex.dwMemoryLoad ) );
		break;
	default:
		break;
	}
