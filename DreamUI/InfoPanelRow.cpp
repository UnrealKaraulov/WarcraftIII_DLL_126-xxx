...
namespace InfoPanel {
	...
	Row::Row(UISimpleFrame* parent, float inHeight, float inItemWidth, unsigned int inItemCount) :
	...
	void Row::setText(unsigned int index, char* format, ...)
	{
		va_list args;
	...
	void Row::setAbsolutePosition(unsigned int originPos, float absoluteX, float absoluteY)
	{
	...
	void Row::update()//MUSTDO Æ¥Åä¿í¶È
	{
		...
		for (std::map<unsigned int, std::string>::iterator iter = contentText.begin();
