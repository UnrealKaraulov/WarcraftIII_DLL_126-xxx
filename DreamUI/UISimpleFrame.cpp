...
void UISimpleFrame::setParent(UISimpleFrame* parent) {
	...
	if ((parentBaseObj = parent->base<war3::CSimpleFrame*>()) && parent != this) {
		dreamaero::generic_this_call<unsigned int>(Offset(SIMPLEFRAME_SETPARENT), baseObj, parentBaseObj);
	}
}
...
void UISimpleFrame::setLevel(int level) {
	war3::CSimpleFrame* baseObj = this->base<war3::CSimpleFrame*>();
	dreamaero::generic_this_call<unsigned int>(Offset(SIMPLEFRAME_SETLEVEL), baseObj, level);
...
UILayoutFrame* UISimpleFrame::parent()
{
	...
	if (baseObj)
	{
		return new UILayoutFrame(baseObj->parent);
	}
	return NULL;
}
