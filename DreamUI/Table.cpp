...
Table::~Table()
{
	for (unsigned int i = 0; i < elements.size(); ++i) //删除所有行的所有元素
	{
...
void Table::setRowColumnCount(unsigned int rowCount, unsigned int columnCount)
{
	...
	{
		for (unsigned int i = rowCount; i < min(this->rowCount, elements.size()); ++i) //删除多余行的所有元素
		{
			for (unsigned int j = 0; j < elements[i].size(); ++j)
			{
				if (elements[i][j]) delete elements[i][j];
	...
	{
		for (unsigned int i = 0; i < min(rowCount, elements.size()); ++i) //删除剩余行的多余列元素
		{
...
void Table::refresh()
{
	...
	IUIObject* elem;
	for (unsigned int i = 0; i < rowCount; i++) //第i行
	{
		for (unsigned int j = 0; j < columnCount; j++) //第j列
		{
			elem = elements[i][j];
			if (elem)
			{
