...
int vsyncEnabled = false;

int vsyncInitialized = false;

int OLD_D3D_PARAMETERS_LOADED = false;

D3DCAPS8 caps8;


unsigned long GlobalMultiSampleType = 0xFFFF;
...
typedef int(__thiscall* Wc3SetTextureStageState)(int a1, unsigned long Stage, D3DTEXTURESTAGESTATETYPE Type, unsigned long Value);
Wc3SetTextureStageState Wc3SetTextureStageState_org;
Wc3SetTextureStageState Wc3SetTextureStageState_ptr = NULL;


unsigned long MipMapCount = 0;
...
void __stdcall EnableAntialiasing(unsigned long value)
{
...
typedef HRESULT(__stdcall* DrawIndexedPrimitiveDx8p)(D3DPRIMITIVETYPE PrimitiveType, unsigned int MinIndex, unsigned int NumVertices, unsigned int StartIndex, unsigned int PrimitiveCount);
DrawIndexedPrimitiveDx8p DrawIndexedPrimitiveDx8_org;


HRESULT DrawIndexedPrimitiveDx8(D3DPRIMITIVETYPE PrimitiveType, unsigned int MinIndex, unsigned int NumVertices, unsigned int StartIndex, unsigned int PrimitiveCount)
{
	HRESULT retval;
...
void DrawOverlayDx8()
{
	...
	for (auto& img : ListOfRawImages)
	{
		...
		{
			rv = d->CreateTexture((unsigned int)img.width, (unsigned int)img.height, 0, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &ppTexture);
