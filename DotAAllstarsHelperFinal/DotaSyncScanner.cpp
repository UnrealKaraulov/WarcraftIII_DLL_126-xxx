...
std::vector<ScanStructUnit> ScanUnitsForDump;
std::vector<ScanStructItem> ScanItemsForDump;

std::vector<ScanStructUnit> ScanUnitsForDump_fromdump;
std::vector<ScanStructItem> ScanItemsForDump_fromdump;
...
void FillScanList()
{
	...
	unsigned char** itemsarray = 0;
	int* ItemsCount = GetItemCountAndItemArray(&itemsarray);
	if (ItemsCount && *ItemsCount > 0 && itemsarray)
	{
	...
	unsigned char** unitsarray = 0;
	int* UnitsCount = GetUnitCountAndUnitArray(&unitsarray);
	if (UnitsCount && *UnitsCount > 0 && unitsarray)
	{
...
int __stdcall ReplayDesyncScan(int scan_or_check)
{
