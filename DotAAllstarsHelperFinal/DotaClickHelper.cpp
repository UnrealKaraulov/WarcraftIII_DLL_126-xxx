...
+ *
+ * @param VK код виртуальной клавиши, которую нужно нажать
+ *
+ * @return void
+ */
...
/* GetTestValue(0) - если значение меняется - значит обработчик хоткеев работает и доступен
GetTestValue(1) - если не меняется, то включена функция блокировки ввода
GetTestValue(2) - если не меняется то чат активен, или же отправляется повтор хоткеев (нажатие и удержание)
GetTestValue(3) - если не меняется, то окно свернуто
GetTestValue(4) - если не меняется то активирован пропуск сообщение в SkipMessagesList, или же включена блокировка мыши
GetTestValue(5) - если не меняется то активен чат или же не активен главный фрейм игры
GetTestValue(6) - если меняется то вызвыаются хоткеи в ProcessHotkey
GetTestValue(7) - не сделано, но должно было проверять если вызываются ProcessKeyCallbackActions*/
