...
public:
	...
	inline const unsigned char* GetRawResponseContent(void);
	...
	inline bool SetAdditionalDataToSend(unsigned char* data, unsigned int dataSize);
	...
	inline unsigned long GetLastError(void);
...
bool WinHttpClient::SendHttpRequest(const std::wstring& httpVerb, bool disableAutoRedirect)
{
	...
	if (::WinHttpCrackUrl(m_requestURL.c_str(), m_requestURL.size(), 0, &urlComp))
	{
		...
		if (hConnect != NULL)
		{
			unsigned long dwOpenRequestFlag = (urlComp.nScheme == INTERNET_SCHEME_HTTPS) ? WINHTTP_FLAG_SECURE : 0;
			...
			if (hRequest != NULL)
			{
				...
				{
					unsigned long options = SECURITY_FLAG_IGNORE_CERT_CN_INVALID
				...
				while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
				{
					...
					{
						unsigned long dwDisableFeature = WINHTTP_DISABLE_REDIRECTS;
					...
					if (bSendRequestSucceed)
					{
						...
						if (::WinHttpReceiveResponse(hRequest, NULL))
						{
							...
							if (bResult || (!bResult && (::GetLastError() == ERROR_INSUFFICIENT_BUFFER)))
							{
								...
								if (szHeader != NULL)
								{
										...
										WINHTTP_NO_HEADER_INDEX))
									{
										...
										std::wsmatch result;
										std::wregex regExp(L"charset=([A-Za-z0-9\\-_]+)");

										if (!m_bForceCharset)
										{
											regExp = std::wregex(L"charset=([A-Za-z0-9\\-_]+)");
										...
										regExp = std::wregex(L"Content-Length:\\s+([0-9]+)");
										if (std::regex_match(m_responseHeader, result, regExp) && result.size() > 0)
										{
											m_responseByteCount = (unsigned int)_wtoi(result[1].str().c_str());
										}

										regExp = std::wregex(L"Location:\\s+([0-9]+)");
										if (std::regex_match(m_responseHeader, result, regExp) && result.size() > 0)
										{
											m_location = result[1];
										}

										regExp = std::wregex(L"Set-Cookie:\\s*(.+?)\\n");
										if (std::regex_match(m_responseHeader, result, regExp) && result.size() > 0)
										{
											std::wstring tmpstr = m_responseHeader;
							...
							m_pResponse = new unsigned char[iMaxBufferSize];
							if (m_pResponse == NULL)
							{
								bRetVal = false;
								break;
							...
							do
							{
								...
								if (::WinHttpQueryDataAvailable(hRequest, &dwSize))
								{
									...
									unsigned char* pResponse = new unsigned char[dwSize + 1];
									if (pResponse != NULL)
									{
										memset(pResponse, 0, (dwSize + 1) * sizeof(unsigned char));
											...
											&dwRead))
										{
											...
											{
												unsigned char* pOldBuffer = m_pResponse;
												m_pResponse = new unsigned char[iMaxBufferSize * 2];
												if (m_pResponse == NULL)
												{
							...
							unsigned long dwFlag = MB_PRECOMPOSED;
...
bool WinHttpClient::SetAdditionalDataToSend(unsigned char* data, unsigned int dataSize)
{
	...
	m_pDataToSend = NULL;
	m_pDataToSend = new unsigned char[dataSize];
	if (m_pDataToSend != NULL)
	{
...
const unsigned char* WinHttpClient::GetRawResponseContent(void)
{
	return m_pResponse;
}
...
unsigned long WinHttpClient::GetLastError(void)
{
