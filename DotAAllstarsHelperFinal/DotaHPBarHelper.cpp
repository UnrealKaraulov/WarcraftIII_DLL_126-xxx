...
std::vector<CustomHPBar> CustomHPBarList[20];
...
int __stdcall SetColorForUnit(unsigned int* coloraddr, BarStruct* BarStruct)
{
	...
	if (BarStruct->_BarClass != _BarVTable && BarStruct->_BarClass != (unsigned char*)BarVtableClone)
	{
		return retval;
	}
	else
	...
	unsigned char* unitaddr = BarStruct->unitaddr;
