...
std::vector<CFrameBuffer> CFrameBufferList;
...
std::string GetCallbackFuncName(CWar3Frame* frame)
{
	if (!frame)
		return std::string();
	for (auto s : CFrameBufferList)
...
unsigned char* __stdcall CFrame_GetFrameAddress(CWar3Frame* frame)
{
	return frame->FrameAddr;
}

// Load custom FrameDef.toc file ( .txt file with fdf path list)
void __stdcall CFrame_LoadFramesListFile(const char* path_to_listfile, int forcereload)
{
...
CWar3Frame* __stdcall CFrame_CreateNewFrame(const char* FrameName, unsigned char* relativeframeaddr, int show, int FrameId)
{
...
void __stdcall CFrame_SetFrameTexture(CWar3Frame* frame, const char* texturepath, const char* borderpath, int tiled)
{
	if (!frame)
		return;
...
void __stdcall CFrame_Enable(CWar3Frame* frame, int enabled)
{
	if (!frame)
		return;
...
int __stdcall CFrame_IsEnabled(CWar3Frame* frame)
{
	if (!frame)
		return false;
...
void __stdcall CFrame_SetScale(CWar3Frame* frame, CFrameBackdropType backtype, int FillToFrame, float scalex, float scaley)
{
	if (!frame)
		return;
...
void __stdcall CFrame_Show(CWar3Frame* frame, int show)
{
	if (!frame)
		return;
	frame->Show(show);
